...
def dashboard_projectdata
  ...
  @projects.each do |p|
    ...
    @kosten = Timetrack.select("mobject_id, sum(amount) as summe").where('mobject_id=? and costortime=?',p.id, "Kosten").group("mobject_id")
    @aufwand = Timetrack.select("mobject_id, sum(amount) as summe").where('mobject_id=? and costortime=?',p.id, "Aufwand").group("mobject_id")
    if @kosten.first 
