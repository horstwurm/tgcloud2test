class PartnerLinksController < ApplicationController
  ...
  def create
    ...
    if @partner_link.save
      redirect_to company_path(:id => @partner_link.company, :topic => "Links (Partner)"), notice: 'Partner link was successfully created.'
    else
      render :new
    end
  end
  ...
  def update
    if @partner_link.update(partner_link_params)
      redirect_to company_path(:id => @partner_link.company, :topic => "Links (Partner)"), notice: 'Partner link was successfully updated.'
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    @partner_link.destroy
      redirect_to company_path(:id => @comp, :topic => "Links (Partner)"), notice: 'Partner link was successfully destroyed.'
  end
